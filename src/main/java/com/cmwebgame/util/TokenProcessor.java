...
public class TokenProcessor {
	private long privious;//上次生成表单标识号得时间值
	    ...
	    */
	   public synchronized boolean isTokenValid(RequestContext request){
	       //未避免session对象不存在时候创建Session对象
		   SessionContext session=request.getSessionContext(false);
	    ...
	    */
	   public synchronized void reset(RequestContext request){
		   SessionContext session=request.getSessionContext(false);
	       if(session==null){
	           return;
	       }
